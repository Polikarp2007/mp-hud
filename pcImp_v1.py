...
class _SliderBadge(QWidget):
    """Small rectangle with a smooth sin-wave sliding white bar (wall-clock based)."""
    def __init__(self, parent=None):
        super().__init__(parent)
        self.setFixedSize(30, 5)
        self._t0 = None
        self._timer = QTimer()
    ...
    def paintEvent(self, event):
        ...
        t = (time.monotonic() - self._t0) * 3.0  # period ~2.1s
        p = QPainter(self)
        ...
        p.drawRoundedRect(QRectF(bar_x, 0, bar_w, h), 2, 2)
        p.end()


class _TrackCell(QWidget):
    """One grid row: draws the vertical line segment + station dot."""
    _SEG_NONE   = 'none'
    _SEG_DIM    = 'dim'
    _SEG_SOLID  = 'solid'
    _SEG_DASHED = 'dashed'
    ...
    def set_dot_blink(self, alpha: int):
        self._dot_color = QColor(255, 255, 255, alpha)
        self.update()
    ...
    def paintEvent(self, event):
        ...
        def draw_seg(y0, y1, style):
            ...
            if style == _TrackCell._SEG_DIM:
                ...
                pen = QPen(QColor(255, 255, 255, 210), 2)
            p.setPen(pen)
            p.drawLine(QPointF(cx, float(y0)), QPointF(cx, float(y1)))

        # Draw line through the dot center — dot drawn on top covers the overlap
        draw_seg(0, cy, self._seg_above)
        draw_seg(cy, self.height(), self._seg_below)

        # Dot on top, opaque, covers the line at this station point
        p.setPen(Qt.PenStyle.NoPen)
...
class Overlay(QWidget):
    ...
    _hud_data_ready = pyqtSignal(dict)
    _steam_update = pyqtSignal(str, bytes)
    ...
    def __init__(self):
        ...
        self.key_signal.connect(self.trigger_key_logic)
        self._hud_data_ready.connect(self.refresh_timetable)
        self._steam_update.connect(self._apply_steam_profile)
        self._dot_color_hex = "#2ecc71"
    ...
    def initUI(self):
        ...
        self.icon_label.setPixmap(self.create_circular_pixmap(resource_path("icon.png"), 45))
        ...
        import os
        _player_svg = resource_path("player.svg")
        if os.path.exists(_player_svg):
            renderer = QtSvg.QSvgRenderer(_player_svg)
        ...
        # Station blink timer — smooth sin-wave at 30fps
        ...
        self._station_blink_timer.start(16)
    ...
    def _apply_steam_profile(self, name, img_data):
        pixmap = QPixmap()
    ...
    def setup_chat_panel(self):
        ...
        _send_svg = resource_path("send.svg")
        if os.path.exists(_send_svg):
            self.send_btn.setIcon(QIcon(_send_svg))
    ...
    def setup_right_panel(self):
        ...
        train_icon = get_svg_label(resource_path("train.svg"), 16)
        ...
        route_icon = get_svg_label(resource_path("route.svg"), 16)
        ...
        dst_icon = get_svg_label(resource_path("metr.svg"), 14)
        ...
        self.tt_grid.setVerticalSpacing(0)
    ...
    def pulse_dot(self):
        ...
        _dot_c = QColor(getattr(self, '_dot_color_hex', '#2ecc71'))
        self.status_dot.setStyleSheet(f"""
            background-color: {getattr(self, '_dot_color_hex', '#2ecc71')};
        ...
        self.dot_shadow.setColor(QColor(_dot_c.red(), _dot_c.green(), _dot_c.blue(), alpha))



    def _blink_current_station(self):
        import time
        if not hasattr(self, '_blink_t0') or self._blink_t0 is None:
            self._blink_t0 = time.monotonic()
        t = (math.sin((time.monotonic() - self._blink_t0) * 2.5) + 1) / 2  # period ~2.5s
        alpha = int(t * 185 + 40)  # 40..225
        cell = getattr(self, '_current_track_cell', None)
        if cell is not None:
            try:
                cell.set_dot_blink(alpha)
            except RuntimeError:
                self._current_track_cell = None
        effect = getattr(self, '_station_label_effect', None)
        if effect is not None:
            try:
                effect.setOpacity(t * 0.7 + 0.3)
            except RuntimeError:
                self._station_label_effect = None
        ...
        def _worker():
            try:
                ...
                if response.status_code == 200:
                    self._dot_color_hex = "#2ecc71"
                    self._hud_data_ready.emit(response.json())
                else:
                    print(f"Ошибка сервера: {response.status_code}")
                    self._dot_color_hex = "#f39c12"
            except Exception as e:
                print(f"Ошибка подключения: {e}")
                self._dot_color_hex = "#e74c3c"
            finally:
                self._fetch_in_progress = False
        threading.Thread(target=_worker, daemon=True).start()
    ...
    def refresh_timetable(self, data):
        ...
        if not route_from or not route_to:
            _rparts = data.get("route", "").split(" to ", 1)
            if len(_rparts) == 2:
                route_from = route_from or _rparts[0].strip()
                route_to = route_to or _rparts[1].strip()
        ...
        _prev_station_name = getattr(self, '_blink_station_name', None)
        _prev_badge_t0 = None
        old_badge = getattr(self, '_current_shimmer_badge', None)
        if old_badge is not None:
            try:
                _prev_badge_t0 = old_badge._t0
                old_badge.stop()
            except RuntimeError:
                pass
        ...
        for i, (name, arr, dep) in enumerate(stations):
            ...
            if status == 'passed':
                dot_color = QColor(255, 255, 255, 55)
            elif status == 'current':
                dot_color = QColor(255, 255, 255, 220)  # white, will blink via timer
            elif status == 'next':
            ...
            self.tt_grid.addWidget(cell, i, 1)

            if status == 'current':
                self._current_track_cell = cell
                if _prev_station_name != name:
                    self._blink_t0 = None
                self._blink_station_name = name

            # ── Time column ───────────────────────────────────────────────────
            time_container = QWidget()
            time_v = QVBoxLayout(time_container)
            time_v.setContentsMargins(0, 10, 0, 10)
            time_v.setSpacing(0)
            ...
            if status == 'passed':
                ...
                st_l = QLabel(name)
                _f = st_l.font(); _f.setUnderline(True); st_l.setFont(_f)
                st_l.setStyleSheet(
                    "color: white; font-family: 'Segoe UI';"
                    " font-size: 12px; font-weight: 800; border: none;"
                )
                eff = QGraphicsOpacityEffect(); eff.setOpacity(1.0)
                st_l.setGraphicsEffect(eff)
                self._station_label_effect = eff

                badge = _SliderBadge(name_container)
                if _prev_badge_t0 is not None and _prev_station_name == name:
                    badge._t0 = _prev_badge_t0
                self._current_shimmer_badge = badge

                name_lay.addWidget(st_l, 0, Qt.AlignmentFlag.AlignVCenter)
                name_lay.addWidget(badge, 0, Qt.AlignmentFlag.AlignVCenter)
                name_lay.addStretch()
